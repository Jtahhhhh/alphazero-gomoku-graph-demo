...
class RelAttention(nn.Module):
    ...
    def forward(self,x,edges):
        total=torch.zeros_like(x); recorded={}
        for name,q,k,v,e in zip(RELATIONS,self.q,self.k,self.v,edges):
            ...
            msg=(alpha[...,None]*vv).reshape(x.shape[0],-1,self.heads*self.d); agg=torch.zeros_like(x); agg.index_add_(1,dst,msg); total+=agg; recorded[name]=alpha.detach()
...
class RGAT(nn.Module):
    ...
    def __init__(self,board_size=6,hidden_dim=64,attention_heads=4,**_):
        ...
        self.layers=nn.ModuleList(RelAttention(hidden_dim,attention_heads) for _ in range(2)); self.heads=PolicyValueHeads(hidden_dim); self.last_attention={}
    def forward(self,x):
        h=torch.relu(self.input(x.flatten(2).transpose(1,2))); edges=tuple(getattr(self,f"edge_{i}") for i in range(4))
        for layer in self.layers: h,self.last_attention=layer(h,edges)
        return self.heads(h)
