...
- `H1_CONTRACT.md` đã quy định exact player-to-move value, đầy đủ optimal actions, timeout/node-budget không phải ground truth, proofs tách khỏi solver và alignment theo action. Đây là nền hợp lý nhưng chưa mô tả `unknown_reason`, threat certificate hoặc ngân sách threat solver.
- Tests hiện có:
